...
-----------分界线，上述问题已经处理，请忽略------------------
请求后端接口获取数据的页面，支持下拉刷新。

-----------分界线，上述问题已经处理，请忽略------------------
创建文本后，跳回子分类列表时应该重新刷新列表页。
点击笔记查看详情时，数据展示不对。可能是字段取错了，请检查。
点击编辑，标题丢失，可能是字段取错了，请检查。

描述的问题，需要检查其他类似的页面功能，特别是图片内容的逻辑。

-----------分界线，上述问题已经处理，请忽略------------------
1、文本详情页面展示数据不全：点击“文本”内容时，文本详情页面只展示了标题字段，没有展示其他字段，请优化
2、编辑文本时，子分类显示“未指定分类”，应该默认为当前分类。

-----------分界线，上述问题已经处理，请忽略------------------
1、文本详情页面展示数据不全，没有正确展示文本的内容。
2、点击子目录，默认选中图片tab，但如果图片列表为空，但是文本有数据，则选中文本tab。

-----------分界线，上述问题已经处理，请忽略------------------
编辑完文本后回到文本详情应该加载新的内容。其他更新操作同理。
主分类、子分类新增编辑按钮（小笔），不用跳转新页面，在当前卡片编辑即可。
新增逻辑：文本详情页面，双击内容区域进入编辑页面。
问题解答：
1. 编辑时只需要修改分类名称（name），还是也包括描述（description）等其他字段？
  2. 编辑状态下，是否需要"保存"和"取消"按钮？还是直接失焦保存？
  3. 编辑按钮应该显示在卡片的什么位置？
1、只修改分类名称，2、失焦保存，3、名称后面