...
-----------分界线，上述需求已经处理，请忽略------------------
参考content-list.vue，优化collection.vue。collection.vue页面格式应该基本与content-list.vue一致。
 1. Tab 切换：是否需要添加图片/文本的 Tab 切换？需要
  2. 搜索功能：是否需要添加搜索收藏内容的功能？需要
  3. 图片展示：是否需要改为华为图库风格（4列网格，最多显示12张）？需要
  4. 批量操作：是否需要支持长按批量删除收藏？支持，注意按钮需要改为取消收藏
  5. 左滑删除：是否需要支持左滑删除收藏？不需要，图片内容、文本内容在名称后展示爱心图标，都靠右展示，点击后取消收藏，再次点击收藏。支持选中、未选中的不同样式。
  6. 导航栏：是否需要使用 custom-nav-bar 组件？需要。
